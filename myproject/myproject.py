...
from gsuit import GoogleAdminService
...
def auth_required(f):
    ...
    def _verify(*args):
        ...
        Check is user existed by key in header
        """
        auth_headers = request.headers.get('key')
        invalid_msg = {
...
def ch_cam(request):
    """
    :param request: request from app with header 'key'
    :return: Uid of chosen cam
    """
    auth_headers = request.headers.get('key')
    user = db.session.query(User).filter_by(token=str(auth_headers)).first()
    chosen_cam = str(user.cam)
    return chosen_cam
...
def list_cams_desctiption():
    """
    :return: list of dict
    Request to Gsuite, that return information about cams (Uid, IP, port, login, password, room, name, etc.)
    """
    global cams
...
def chose_cam():
    ...
    Stop previous proxing
    Choosing cam from dict of cams
    Proxing new RTSP-stream
    ...
    user = db.session.query(User).filter_by(token=str(auth_headers)).first()
    userid = user.id
    if user.cam is not None:
        try:
            rtspProxiesPool.stop_proxy(str(user.cam))
        except:
            print(user.cam, 'Something was wrong')
    data = request.json
    chosen_cam = data['uid']
    ...
    db.session.query(User).filter(User.id == userid).update({User.cam: chosen_cam})
    db.session.commit()
    return str(port)


@app.route('/release_cam', methods=['GET'])
@auth_required
def release_cam():
    """
    :return: error or successful message
    Stop stream on last used cam
    """
    auth_headers = request.headers.get('key')
    user = db.session.query(User).filter_by(token=str(auth_headers)).first()
    if user.cam is not None:
...
def set_preset():
    ...
    :return: error or successful message
    Set preset of cam
    Due to cam's limitations the max number of preset is 90
     """
    ...
    if db.session.query(Presets.id).filter_by(user_id=userid,
                                              ...
                                              uid_cam=usercam).scalar() is None:

        new_preset = Presets(user_id=userid,
                             uid_cam=int(usercam),
        ...
        print('Change colour later')
        #включить после обновления приложения с добавлением цвета кнопок
        #db.session.query(User).filter(Presets.user_id == userid,Presets.preset_on_cam_id == pres_id,  Presets.uid_cam == chosen_cam).update({Presets.button_clr: data['color']})
        #db.session.commit()
    cam = camss[ch_cam(request)]
...
def goto_preset():
    ...
    auth_headers = request.headers.get('key')
    user = db.session.query(User).filter_by(token=str(auth_headers)).first()
    pres_id = (user.id - 1) * 9 + int(data['go_pres'])
    cam = camss[str(user.cam)]
...
def login():
    ...
    if re.status_code == 202:
        ...
        return json.dumps({'error': 'Something Wrong', 'authenticated': 'false'}), 401
